<?php 
	...
	function addBadge($badgeName, $badgeGoal, $badgeGoalUnits, $badgeDescription, $badgePreReq, $badgeCustomPicUrl) {
		$dbQuery = sprintf("INSERT INTO BADGES (NAME, GOAL, GOAL_UNITS, DESCRIPTION, PRE_REQ, CUSTOM_PIC_URL) VALUES ('%s', '%s', '%s', '%s', '%s', '%s')", mysql_real_escape_string($badgeName), mysql_real_escape_string($badgeGoal), mysql_real_escape_string($badgeGoalUnits),mysql_real_escape_string($badgeDescription), mysql_real_escape_string($badgePreReq), mysql_real_escape_string($badgeCustomPicUrl));
